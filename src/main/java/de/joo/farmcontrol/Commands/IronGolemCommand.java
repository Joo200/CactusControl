...
public class IronGolemCommand implements CommandExecutor {
    ...
    public IronGolemCommand(FarmControlPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        Map<Location, Integer> counterMap = plugin.getIronGolemCounterMap();
        if(counterMap.size() == 0) {
            player.sendMessage(ChatColor.RED + "Es wurden keine Irongolem-Farmen gefunden.");
            return true;
        }
        int page = 1;
        if(strings.length == 1) {
        ...
        player.sendMessage(ChatColor.BLUE + "Irongolemfarmen. Gesamt: " + counterMap.size() + " Seite " + page + " von " + (((int)counterMap.size()/15)+1) +":");
        plugin.getIronGolemCounterMap().entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
