...
public class FishCommand implements CommandExecutor {
    ...
    public FishCommand(FarmControlPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        //Map<Location, Integer> counterMap = plugin.getFishCounterMap();
        Map<Location, Integer> counterMap = plugin.getFishCounterMap().entrySet().stream().filter( p -> p.getValue() > 50).collect(Collectors.toMap(p -> p.getKey(), p -> p.getValue()));
        if(counterMap.size() == 0) {
            player.sendMessage(ChatColor.RED + "Es wurden keine möglichen AFK-Fischfarmen gefunden.");
            return true;
        }
        int page = 1;
        if(strings.length == 1) {
